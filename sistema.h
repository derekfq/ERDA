...
void CadastraPonto(Pontos** ListaDePontos, int reg, int exp) {
    ponto novo;
    novo.regiao = reg;
    novo.exp = exp;
...
void gera_Info(Pontos** ListaDePontos){
    ...
    char nomes_itens[5][20] = { "Dog Xtudao","Dog Milhao","Doginho","Dog Tradicional","Podrao do Tio" };
    ////

    srand(time(NULL));
    //quantidade de pontos e de itens que serao gerados
    qtdd_Pontos = 5;// de 3 a 7
/*
    ...
    {//PARA CADA PONTO
        ...
        qtdd_Pedidos = 5;// de 5 a 15
        for (j = 0; j < qtdd_Pedidos; j++) //FOR PARA CADA PEDIDO
        {
            I = inicializa_Itens();
            qtdd_Itens_Pedido = 5;//QUANTOS ITENS DAQUELE PEDIDO
            ...
            */
            ...
            {
                qtdd_Itens = 5;
                valor_Int = 100; //valor unitario em int
            ...
            printf("%x", I->topo);
            //manda a lista de itens para o pedido do ponto
            printf("\tPEDIDO (j=%d) CADASTRADO!\n", j);
